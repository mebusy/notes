...
Thus, a string of terminals *w* is in L(G), the language generated by G, if and only if *w* is a sentence of G ( or S ⇒<sup>\*</sup> *w*). A language that can be generated by a grammar is said to be a ***context-free language***. If two grammars generate the same language, the grammars are said to be *equivalent*.
